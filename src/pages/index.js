...
export default function Home() {
  return (
    <main className="w-full max-sm:w-screen max-sm:bg-purple-500">
      <Navbar />
      <div className="h-screen px-12 max-sm:px-6 flex justify-between flex-row max-sm:w-screen max-sm:flex-col bg-[#15464b] ">
        <div className="h-full max-sm:py-10 flex w-[35%] max-sm:w-full text-left flex-col justify-center items-start space-y-6">
          <h2 className="text-[#32bab3] max-sm:text-sm font-semibold tracking-widest">
            WELCOME TO FAJAR JAYA
          </h2>
          ...
          <div className="flex flex-row items-center space-x-5">
            <span className="w-fit text-sm px-8 py-3 border bg-transparent text-white border-gray-500 hover:cursor-pointer hover:border-[#32bab3] hover:bg-[#32bab3] hover:text-white transition-all ease-in-out duration-300">
              GET STARTED
            </span>
            <span className="flex flex-row space-x-3 items-center text-[14px] text-[#32bab3] hover:text-white hover:cursor-pointer transition-all duration-300 ease-in-out">
              <p>READ MORE</p>
              <FaArrowRight />
            </span>
          </div>
        </div>
        <div className="h-full bg-[#231f20] w-1/2 max-sm:w-full relative translate-y-[40px] max-sm:translate-y-5">
          <div className="h-24 w-52 max-sm:hidden bg-white absolute top-20 -right-5 flex flex-row p-2 justify-around items-center">
            <FaCheckCircle className="text-5xl text-[#32bab3]" />
